...
***                      Random lattice                           ***
********************************************************************/
/*void rede_random (int **viz, int L)
{
	double RUIDO;
	unsigned long rseed;
	...
	{		
		...
		for(i=1;i<5;i++) // i = direçao do viz
		{
			double l = (int) N*gsl_rng_uniform(rand_vec); //sitio  vizinho
			//double l2 = 1 + (int) 4*gsl_rng_uniform(rand_vec); // direçao do vizinho
